<div>
    ...
    <div class="relative">
        <div class="flex items-center justify-between mb-1">
                ...
                @if($this->searchableColumns()->count())
                    <div class="flex rounded-lg w-96 shadow-sm">
                        <div class="relative flex-grow">
                            ...
                            <input wire:model.debounce.500ms="search" class="block w-full py-3 pl-10 text-sm border-gray-300 leading-4 rounded-md shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50 focus:outline-none" placeholder="{{__('Rechercher par')}} {{ $this->searchableColumns()->map->label->join(', ') }}" type="text" />
                            <div class="absolute z-10 inset-y-0 right-0 flex items-center pr-2">
                                <button wire:click="$set('search', null)" class="text-gray-300 hover:text-red-400
                                focus:outline-none" aria-label="Rechercher">
                                    <x-icons.x-circle class="w-5 h-5 stroke-current text-blue-300" />
                                </button>
                            </div>
                        </div>
                    </div>
            ...
            @if($this->activeFilters)
                <span class="text-xl text-slate-800 uppercase">@lang('Filtre actif')</span>
            @endif
            ...
            <div class="flex flex-wrap items-center space-x-1">
                ...
                @if($this->activeFilters)
                    <button wire:click="clearAllFilters" class="flex items-center px-3 text-xs font-medium tracking-wider text-red-500 uppercase bg-white border border-red-400 space-x-2 rounded-none leading-4 hover:bg-red-200 focus:outline-none" aria-label="Réinitialiser"><span>{{ __('Réinitialiser') }}</span>
                ...
                @if(count($this->massActionsOptions))
                    <div class="flex items-center justify-center space-x-1">
                        <label for="datatables_mass_actions">{{ __('Avec la sélection') }}:</label>
                ...
                @if($exportable)
                    <div x-data="{ init() {
                        ...
                        <button wire:click="export" class="flex items-center px-3 text-xs font-medium tracking-wider text-green-500 uppercase bg-white border border-green-800 space-x-2 rounded-none leading-4 hover:bg-green-200 focus:outline-none"aria-label="Exporter"><span>{{ __('Exporter') }}</span>
                ...
                @foreach ($columnGroups as $name => $group)
                    <button wire:click="toggleGroup('{{ $name }}')"
                            class="px-3 py-2 text-xs font-medium tracking-wider text-green-500 uppercase bg-white border border-green-400 rounded-md leading-4 hover:bg-green-200 focus:outline-none" aria-label="Activer">
                        <span class="flex items-center h-5">{{ isset($this->groupLabels[$name]) ? __($this->groupLabels[$name]) : __('Activer :group', ['group' => $name]) }}</span>
                    </button>
                @endforeach
            ...
            <div class="p-2 grid grid-cols-8 gap-2">
                @foreach($this->columns as $index => $column)
                    @if ($column['hideable'])
                        ...
                        {{ $column['hidden'] ? 'bg-blue-100 hover:bg-blue-300 text-blue-600' : 'bg-blue-500 hover:bg-blue-800' }}" aria-label="{{ $column['label'] }}">
                            {{ $column['label'] }}
                        </button>
                    @endif
                @endforeach
        ...
        <div wire:loading.class="opacity-50" class="rounded-none @unless($complex || $this->hidePagination)
            rounded-b-none @endunless shadow-lg bg-white max-w-screen overflow-x-scroll border-2 border-immogray2
            border-transparent @if($complex) rounded-b-none border-b-0 @endif">
            <div>
                <div class="table min-w-full align-middle">
                    ...
                    @endunless
                    <div class="table-row bg-immopurple divide-x divide-blue-200">
                        @foreach($this->columns as $index => $column)
                            @if($column['hidden'])
                                @if($hideable === 'inline')
            ...
            @if($this->results->isEmpty())
                <p class="p-3 text-lg text-center">
                    {{ __("Il n'y a rien à afficher pour le moment") }}
                </p>
            @endif
        </div>

        @unless($this->hidePagination)
            <div class="max-w-screen bg-white @unless($complex) rounded-b-none @endunless border-4 border-t-0
            border-b-0 @if($this->activeFilters) border-none bg-immogray2 @else border-transparent @endif">
                    ...
                    @if(count($this->results))
                        <div class="flex items-center my-2 sm:my-0">
                            <select name="perPage" class="block w-full py-2 pl-3 pr-10 mt-1 text-base border-gray-300 form-select leading-6 focus:outline-none focus:shadow-outline-blue focus:border-blue-300 sm:text-sm sm:leading-5" wire:model="perPage">
                                ...
                                @endforeach
                                <option value="99999999">{{__('Tout')}}</option>
                            </select>
                        </div>
                        ...
                        <div class="flex justify-end text-gray-600">
                            {{__('Resultats')}} {{ $this->results->firstItem() }} - {{ $this->results->lastItem() }} {{__('sur')}}
                            {{ $this->results->total() }}
                        </div>
                    @endif
                </div>
    ...
    <span class="hidden text-sm text-left text-center text-right text-gray-900 bg-immogray1 bg-yellow-100 leading-5 bg-gray-50"></span>
</div>

<div x-data="{ show: false }" class="flex flex-col items-center">
    <div class="flex flex-col items-center relative">
        <button x-on:click="show = !show" class="px-3 py-2 border border-immopurple rounded-none bg-white text-blue-500 text-xs leading-4 font-medium uppercase tracking-wider hover:bg-blue-200 focus:outline-none" aria-label="{{ __('Afficher/Masquer les colonnes')}}">
            <div class="flex items-center h-5">
                {{ __('Afficher/Masquer les colonnes')}}
            </div>
        </button>
        <div x-show="show" x-on:click.away="show = false" class="z-50 absolute mt-16 -mr-4 shadow-2xl top-100 bg-immopurple w-96 right-0 rounded max-h-select overflow-y-auto" x-cloak>
